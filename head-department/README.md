...
REACT_APP_SERVICE_NAME=Department Head Service
```

## 📝 Notes

1. **Authentication**: The login functionality is a placeholder. In production, integrate with your separate authentication service.

2. **Database**: Ensure the PostgreSQL database has the complete schema with all tables, indexes, and triggers as provided.

3. **JWT Secret**: Change the JWT_SECRET in production to a strong, random value.

4. **CORS**: The backend is configured to accept requests from `http://localhost:3000`. Update FRONTEND_URL in production.

5. **Typo in Schema**: The database schema has a typo: `absence typé` should be `absence_type`. Please fix this in your database.
...
For issues or questions, please refer to the main university platform documentation.

## 📄 License

MIT License - University Platform © 2024
